<?php
// Alternative email solution with detailed error reporting
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Method 1: Try PHPMailer with different settings
    if (!$email_sent) {
        try {
            ...
            // Additional settings to ensure English output
    ...
    // Method 3: Save to file and show success to user
    if (!$email_sent) {
        ...
        // Log the issue
        $log_message = date('Y-m-d H:i:s') . " - Email failed, saved to backup. Errors: " . implode('; ', $error_details) . "\n";
        file_put_contents('email_log.txt', $log_message, FILE_APPEND | LOCK_EX);
        
        // Show success to user but mention email delay
        echo json_encode(['success' => true, 'message' => 'Thank you! Your appointment request has been received. We will contact you soon to confirm your appointment. (Note: Email delivery may be delayed due to technical issues)']);
        exit;
    }
    
    // Log success
    $log_message = date('Y-m-d H:i:s') . " - Email sent successfully\n";
    file_put_contents('email_log.txt', $log_message, FILE_APPEND | LOCK_EX);
